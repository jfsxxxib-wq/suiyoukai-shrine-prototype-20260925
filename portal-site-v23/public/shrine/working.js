(() => {
  ...
  const storageKey = 'suiyoukai-shrine-portal-v1';
  ...
  } catch { /* Invalid saved data leaves a safe empty screen. */ }
  ...
  $('copy-result').addEventListener('click', () => doAction(() => {
    ...
    void copyText(core.readableResult(output), $('transfer-text'));
  }));
  ...
  $('saved-results').addEventListener('click', event => {
    ...
    if (result) void copyText(core.readableResult(result), $('transfer-text'));
  });
  $('export-data').addEventListener('click', () => doAction(() => {
    void copyText(core.exportReadableData(state.roster, state.results), $('transfer-text'));
  }));
  $('import-data').addEventListener('click', () => doAction(() => {
    const source = $('transfer-text').value;
    const data = core.importTransferText(source);
